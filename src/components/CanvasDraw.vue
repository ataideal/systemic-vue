...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.channel.join()
        .receive("ok", ({points}) => {
          this.history = points.slice();
          // this.history.forEach((point, i) => {this.draw.draw(point,i)})
          this.draw.redraw()
        })
        .receive("error", ({reason}) => console.log("failed join", reason) )
        .receive("timeout", () => console.log("Networking issue. Still waiting..."))

      this.channel.on("new_point", (point) => {
        this.history.push(point.point);
        this.draw.draw(point.point,this.history.length);
      })

      this.channel.on("erase", () => {
        this.history = []
        this.draw.redraw()
      })
    ...
    watch:{
      sendPoint(val){
        if(val!= null)
            this.channel.push("create_point",{point:val},1000)
      }
    },
    methods: {
      setEraser(){
      ...
      removeAllHistory(){
        //this.history = [];
        this.channel.push("destroy_points",{},1000)
        //this.draw.redraw();
      },
      simplify (){
        var simpleHistory = [];
  ...
  class Draw {
    ...
    setSize(){
      this.c.width = window.innerWidth - 280;
    ...
    moveMouse(e){
      ...
      var cursor = this.app.$refs.cursor;
    ...
    draw(item, i){
      ...
      var lastItem = this.app.history[i-1];
      if(i < 2){
        return false;
      }

      if(!item.isDummy && !this.app.history[i-2].isDummy && !prevItem.isDummy){
